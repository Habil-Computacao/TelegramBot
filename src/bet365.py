...
class ApiBet365:
    ...
    def _parse_teams(self, data: str) -> Tuple[list, list]:
    ...
    def _full_time_result(self, data: str) -> List:
        ...
        if not (len(events) == len(full_time_result) / 3):
            return []
        ...
        # new foramt
        le = len(events)
        if not (le == len(full_time_result) / 3):
            return []
    ...
    def _both_teams_to_score(self, data: str) -> List:
        ...
        # new foramt
        if not (len(events) == len(both_teams_to_score) / 2):
            return []
    ...
    @staticmethod
    def _request(
            s: requests.Session, competition: str, category: int
    ) -> str:
    ...
    def _requests(self, competition: str) -> Tuple[str, str, str]:
